...
class Question(models.Model):
    ...
    category = models.ForeignKey(
        ...
        verbose_name='Categoria da pergunta',
    )
    level = models.CharField(
        'Nível da pergunta', max_length=1, choices=LEVEL_CHOICES
    )
    active = models.BooleanField(
        'Pergunta está ativa?', default=True
    )  # Define se a questão está ou não ativa para ser utilizada
...
class Option(models.Model):
    question = models.ForeignKey(
        Question, on_delete=models.CASCADE, verbose_name='Pergunta'
    )
    text = models.CharField('Texto da opção de resposta', max_length=200)
    answer = models.BooleanField(
        'Resposta correta?', default=False
    )  # Define se a pergunta é a resposta da questão
...
class Match(models.Model):
    ...
    level = models.CharField(
        'Nível das perguntas', max_length=1, choices=LEVEL_CHOICES
    )
    ...
    def get_user_score(self, user):
        ...
        return (
            UserAnswer.objects.filter(
                match_answer=self, user=user, option__answer=True
            )
            .distinct('question')
            .count()
        )
    ...
    def get_user_questions_answered(self, user):
        ...
        '''
        return (
            UserAnswer.objects.filter(match_answer=self, user=user)
            .distinct('question')
            .count()
        )
    ...
    def user_questions_to_answer(self, user):
        ...
        '''
        return (
            self.questions.exclude(
                useranswer__user=user, useranswer__match_answer=self
            )
    ...
    def is_ready_to_finish(self):
        '''
        Verifica se a partida pode ser finalizada

        Uma partida pode ser finalizada se todos os jogadores já responderam
        suas perguntas
        '''
...
class UserAnswer(models.Model):
    ...
    def judgment(self):
        '''
        Retorna um boleano julgando a resposta da pergunta.

        True se a resposta da pergunta estiver correta.
        False se estiver incorreta ou a resposta expirou.
        '''
        return (
            True
            if self.option and self.option == self.question.answer
            else False
        )
...
class UserProfile(models.Model):
    ...
    def pending_matches(self):
        '''
        Retorna as partidas pendentes do usuário.

        São partidas que ainda possuem perguntas a serem respondidas.

        Returns:
            QuerySet[Match]: partidas pendentes do usuário.
        '''
        ...
        user_pending_matches = user_matches.filter(
            pk__in=user_pending_matches_list
        )
    ...
    def finished_matches(self):
        '''
        Retorna as partidas finalizadas do usuário.

        Returns:
            QuerySet[Match]: partidas finalizadas do usuário.
        '''
        user_matches = self.matches
        ...
        user_finished_matches = user_matches.filter(
            pk__in=user_finished_matches_list
        )
    ...
    def in_progress_matches(self):
        '''
        Retorna um queryset das partidas aguardando finalização dos oponentes
        '''
        user_matches = self.matches
        pending_matches = self.pending_matches
        finished_matches = self.finished_matches
        in_progress = user_matches.exclude(pk__in=pending_matches).exclude(
            pk__in=finished_matches
        )
        return in_progress

    def min_position_matches(self, min_position):
        '''
        Retorna as partidas em que o usuário ficou acima da posição recebida.

        Returns:
            QuerySet[Match]: partidas do usuário acima de min_position.
        '''
        user_matches = self.matches
    ...
    def podium_matches(self):
        '''Retorna as partidas em que o usuário ficou no pódio'''
...
def user_answer_post_save(sender, instance, **kwargs):
    '''
    Salva a hora e vencedor, quando a ultima respota é salva na partida.
    '''
    ...
    if match.is_ready_to_finish and not match.end_date and not match.winner:
        Match.objects.filter(pk=match.pk).update(
            end_date=timezone.now(), winner=match.get_winner()[0]
        )
